...
interface QuotePrintClientProps {
  quote: any;
  branding: any;
  orgName: string;
  items: any[];
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <h1 className="text-4xl font-extrabold tracking-tight">QUOTATION</h1>
                    <p className="text-sm font-semibold opacity-90">REF: PRO-{quote.id.slice(4, 12).toUpperCase()}</p>
                  </div>
                  <p className="text-xs opacity-75 pt-2">{appName}, {new Date(quote.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p>
                </div>
                ...
                <div className="space-y-3.5 text-xs text-right md:self-end">
                  <div>
                    ...
                    <p className="font-semibold text-sm mt-0.5">
                      {new Date(quote.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "2-digit", year: "numeric" })}
                    </p>
                  </div>
                  <div>
                    <p className="opacity-75 uppercase tracking-wider text-[9px] font-mono">Validity Period</p>
                    <p className="font-semibold text-sm mt-0.5">30 Days (Valid until Date)</p>
                  </div>
                  <div>
                    <p className="opacity-75 uppercase tracking-wider text-[9px] font-mono">PREPARED FOR:</p>
                    <p className="font-bold text-sm mt-0.5">{quote.client?.name}</p>
                    {quote.client?.company && (
                      <p className="opacity-90">{quote.client.company.name}</p>
                    )}
                  </div>
                  <div className="pt-1">
                    <p className="font-bold text-sm">Estimated Total: ₹{grandTotal.toLocaleString("en-IN")}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-8 md:p-10 pt-10 grid grid-cols-1 md:grid-cols-12 gap-8 items-end avoid-break">
              <div className="md:col-span-7 space-y-6 text-xs text-slate-500">
                ...
                <div className="space-y-1">
                  ...
                  <ul className="list-disc pl-3 text-muted-foreground text-[10px] space-y-1">
                    <li>Implementation includes 3 months core hypercare support post-launch.</li>
                    <li>API limits are capped at 50,000 requests/day unless upgraded.</li>
                    <li>Pricing excludes custom local taxation parameters which may apply.</li>
                  </ul>
                </div>
              ...
              <div className="md:col-span-5 space-y-6 text-right ml-auto w-full max-w-[320px]">
                ...
                <div className="space-y-2 pt-4">
                  ...
                  <div className="inline-block text-right border-t border-slate-200 pt-1.5 w-48 font-mono text-[9px] uppercase tracking-wider text-slate-400">
                    ...
                    <p className="mt-0.5">Commercial Manager</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <>
            <div className="p-8 md:p-10 pb-0">
              ...
              <div className="grid grid-cols-12 items-center gap-1 my-8">
                <div className="col-span-4 h-8 rounded-l-sm" style={{ backgroundColor: primaryColor }} />
                <div className="col-span-4 text-center">
                  <h1 className="text-xl font-black text-slate-800 uppercase tracking-widest leading-none">QUOTATION</h1>
                </div>
                <div className="col-span-4 h-8 rounded-r-sm" style={{ backgroundColor: primaryColor }} />
              </div>

              {/* Bill to & Invoice Meta Info */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs pb-4">
                <div className="space-y-1.5">
                  <h3 className="font-bold text-slate-900 text-sm">Prepared for:</h3>
                  <p className="font-extrabold text-slate-800 text-sm">{quote.client?.name}</p>
                  <p className="text-slate-500 leading-relaxed max-w-xs">
                    {quote.client?.company?.name || "Client Partner Workspace"},<br />
                ...
                <div className="md:text-right space-y-2 md:ml-auto max-w-[240px]">
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-slate-600">
                    <span className="font-bold text-slate-900 text-left md:text-right">Quote Ref#</span>
                    <span className="font-mono text-slate-800 font-semibold">PRO-{quote.id.slice(4, 12).toUpperCase()}</span>
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-800 text-white font-bold font-mono text-[9px] tracking-wider uppercase border-b border-slate-800">
                      ...
                      <th className="p-3 pl-4">Scope / Deliverable Description</th>
          ...
          <div className="relative">
            ...
            <div className="pt-28 px-8 md:px-10 pb-4 flex justify-between items-start">
              ...
              <div className="text-right">
                <h1 className="text-4xl font-extrabold uppercase tracking-widest leading-none" style={{ color: primaryColor }}>QUOTATION</h1>
              </div>
            </div>
            ...
            <div className="px-8 md:px-10 py-4 flex justify-end">
              <div className="text-right space-y-1.5">
                ...
                <p className="font-extrabold text-slate-800 text-sm mt-0.5">{quote.client?.name}</p>
            ...
            <div className="px-8 md:px-10 py-4 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
              ...
              <div className="md:col-span-7 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-y-1 text-[11px] font-mono text-slate-600 leading-normal">
                  ...
                  <span className="col-span-2 text-slate-800 font-semibold">PRO-{quote.id.slice(4, 12).toUpperCase()}</span>
                  ...
                  <span className="col-span-2 text-slate-800 font-semibold">30 Days</span>
                </div>
              </div>
              ...
              <div className="md:col-span-5 bg-slate-700 text-white px-5 py-4 rounded-sm flex justify-between items-center text-sm font-bold font-mono shadow-sm">
                <span className="text-xs uppercase tracking-wider text-slate-300">Total Estimate:</span>
                <span>₹{grandTotal.toLocaleString("en-IN")}</span>
              </div>
            </div>
            ...
            <div className="px-8 md:px-10 py-6">
              <div className="border border-slate-200 rounded-sm overflow-hidden shadow-sm/5">
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr
                      className="text-white font-bold font-mono text-[9px] tracking-wider uppercase border-b"
                      style={{ backgroundColor: primaryColor, borderBottomColor: adjustColor(primaryColor, -15) }}
                    >
